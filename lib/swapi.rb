...
sw.get_people
sw.get_people_attr
sw.people.each do |key, value|
  print value['vehicles']
  print "\n"
  print value['starships']
end

# HTTParty.get("http://swapi.co/api/people/1").parsed_response['vehicles'].each do |url|
#   puts  HTTParty.get("http://swapi.co/api/vehicles/#{url.match(/(\d+)/)}").parsed_response['name']
# end

# print HTTParty.get("http://swapi.co/api/people/1").parsed_response
